    ...
    default_docs = [
        {
            ...
            "title": "Dashboard",
            "sections": [
                {"type": "text", "content": "Dashboard adalah halaman utama yang menampilkan ringkasan aktivitas dan status chatbot secara real-time. Gunakan halaman ini untuk memantau kinerja chatbot setiap hari."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Kartu Statistik Utama", "content": "Empat kartu di bagian atas menampilkan: Total Pesan (masuk + keluar hari ini), Total Kontak (unik), Rules Aktif (aturan yang sedang berjalan), dan AI Calls (panggilan ke AI beserta estimasi token yang digunakan)."},
                {"type": "step", "stepNumber": 2, "stepTitle": "Grafik Aktivitas 7 Hari", "content": "Grafik garis menampilkan tren pesan masuk dan pesan keluar selama 7 hari terakhir. Gunakan ini untuk melihat pola waktu ramai dan mengidentifikasi lonjakan aktivitas."},
                {"type": "step", "stepNumber": 3, "stepTitle": "Status Bot & Koneksi", "content": "Panel Status menampilkan: status bot (Aktif/Nonaktif), status koneksi WhatsApp (CONNECTED/DISCONNECTED), uptime sistem, dan rata-rata waktu respons dalam milidetik."},
                {"type": "step", "stepNumber": 4, "stepTitle": "Workflow Canvas (Live Monitor)", "content": "Visualisasi alur pemrosesan pesan secara real-time. Setiap pesan masuk akan terlihat mengalir: Pesan → Rules Engine → Knowledge Base → AI → Balasan. Node berwarna hijau = sukses, merah = error, abu = idle."},
                {"type": "step", "stepNumber": 5, "stepTitle": "Log Aktivitas Terbaru", "content": "Panel bawah menampilkan 10 event terbaru secara real-time melalui WebSocket. Termasuk pesan masuk, rule yang terpicu, panggilan AI, dan balasan yang dikirim."},
                {"type": "step", "stepNumber": 6, "stepTitle": "Reset Data Dashboard", "content": "Tombol Reset (ikon tempat sampah) tersedia untuk mengosongkan data statistik dashboard. Gunakan dengan hati-hati karena data tidak bisa dipulihkan."},
            ],
            "updatedAt": now_iso,
        },
        {
            ...
            "title": "Lisensi",
            "sections": [
                {"type": "text", "content": "Halaman Lisensi digunakan untuk mengaktifkan lisensi chatbot Anda. Lisensi menentukan masa aktif penggunaan dan paket fitur yang tersedia. Tanpa lisensi aktif, chatbot tidak dapat menerima dan membalas pesan."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Status Lisensi", "content": "Status lisensi ditampilkan di bagian atas: Aktif (hijau) — lisensi valid dan chatbot berjalan normal. Expired (merah) — masa lisensi habis, perbarui untuk melanjutkan. Tidak Aktif — belum ada lisensi yang diaktifkan."},
                {"type": "step", "stepNumber": 2, "stepTitle": "Aktivasi Lisensi", "content": "Masukkan License Key yang diberikan oleh administrator ke kolom aktivasi, lalu klik tombol Aktifkan. License Key berbentuk: XXXX-XXXX-XXXX-XXXX. Pastikan tidak ada spasi di depan atau belakang."},
                {"type": "step", "stepNumber": 3, "stepTitle": "Informasi Lisensi Aktif", "content": "Setelah aktif, panel menampilkan: Nama Pelanggan, Nama Paket (Standard/Premium), Tanggal Kedaluwarsa, Maksimal Aktivasi, dan Jumlah Aktivasi Terpakai."},
                {"type": "step", "stepNumber": 4, "stepTitle": "Perpanjangan Lisensi", "content": "Jika lisensi hampir expired atau sudah expired, hubungi administrator untuk mendapatkan license key baru. Masukkan key baru dan klik Aktifkan untuk memperbarui lisensi."},
                {"type": "step", "stepNumber": 5, "stepTitle": "Lisensi Otomatis (User Baru)", "content": "Jika akun Anda dibuat oleh superadmin dan sudah dilengkapi lisensi, lisensi akan otomatis aktif tanpa perlu aktivasi manual."},
            ],
            "updatedAt": now_iso,
        },
        {
            ...
            "title": "Koneksi",
            "sections": [
                {"type": "text", "content": "Halaman Koneksi digunakan untuk menghubungkan chatbot ke WhatsApp melalui WAHA (WhatsApp HTTP API). Koneksi yang stabil diperlukan agar chatbot bisa menerima dan mengirim pesan."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Mode Koneksi: Managed", "content": "Jika akun Anda dibuat oleh superadmin, koneksi WAHA sudah ter-assign otomatis dari pool server. Anda hanya perlu scan QR Code tanpa perlu mengisi URL atau API Key secara manual."},
                {"type": "step", "stepNumber": 2, "stepTitle": "Mode Koneksi: Manual", "content": "Untuk koneksi manual: isi URL Server WAHA (contoh: https://waha.domainanda.com), isi API Key jika server menggunakan autentikasi, dan isi Nama Session (default: 'default'). Satu session = satu nomor WhatsApp."},
                {"type": "step", "stepNumber": 3, "stepTitle": "Simpan Konfigurasi", "content": "Klik tombol Simpan setelah mengisi URL, API Key, dan nama session. Sistem akan menyimpan konfigurasi dan mencoba terhubung ke server WAHA."},
                {"type": "step", "stepNumber": 4, "stepTitle": "Scan QR Code", "content": "Setelah konfigurasi disimpan, klik tombol Mulai Session. QR Code akan muncul — scan menggunakan aplikasi WhatsApp di HP Anda: Menu → Perangkat Tertaut → Tautkan Perangkat."},
                {"type": "step", "stepNumber": 5, "stepTitle": "Status Koneksi", "content": "Panel status menampilkan kondisi real-time: CONNECTED (hijau) — siap menerima pesan. SCAN_QR_CODE — menunggu scan. STARTING — sedang inisialisasi. STOPPED — session berhenti, klik Mulai untuk memulai ulang."},
                {"type": "step", "stepNumber": 6, "stepTitle": "Webhook Otomatis", "content": "Setelah koneksi berhasil, sistem otomatis mendaftarkan webhook ke WAHA agar setiap pesan masuk langsung diteruskan ke chatbot. Tidak perlu konfigurasi webhook manual."},
                {"type": "step", "stepNumber": 7, "stepTitle": "Troubleshooting", "content": "Jika status tidak berubah ke CONNECTED setelah scan: pastikan koneksi internet HP stabil, coba klik Stop lalu Mulai Session lagi, atau refresh halaman dan scan ulang QR yang baru muncul."},
            ],
            "updatedAt": now_iso,
        },
        {
            ...
            "title": "AI Agent",
            "sections": [
                {"type": "text", "content": "AI Agent adalah otak chatbot Anda. Di sini Anda mengatur provider AI, model yang digunakan, kepribadian chatbot, informasi bisnis, dan parameter respons. Konfigurasi yang baik menghasilkan chatbot yang akurat dan natural."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Pilih Provider AI", "content": "Tersedia tiga provider: OpenAI (GPT-4o, GPT-4o-mini — terbaik untuk akurasi), Google Gemini (Gemini 2.0 Flash — cepat dan hemat), Anthropic Claude (Claude 3.5 Sonnet — sangat baik untuk bahasa natural), dan Ollama (model lokal — gratis, privat, tapi butuh server sendiri)."},
                {"type": "step", "stepNumber": 2, "stepTitle": "API Key", "content": "Masukkan API Key sesuai provider yang dipilih. OpenAI: dapatkan di platform.openai.com. Gemini: dapatkan di aistudio.google.com. Anthropic: dapatkan di console.anthropic.com. API Key disimpan terenkripsi dan tidak pernah ditampilkan kembali."},
                {"type": "step", "stepNumber": 3, "stepTitle": "Pilih Model", "content": "Setiap provider menawarkan model berbeda dengan kemampuan dan biaya yang berbeda. Rekomendasi: GPT-4o-mini atau Gemini 2.0 Flash untuk keseimbangan kecepatan dan biaya. GPT-4o atau Claude 3.5 Sonnet untuk akurasi maksimal."},
                {"type": "step", "stepNumber": 4, "stepTitle": "System Prompt", "content": "Tulis instruksi kepribadian dan peran chatbot. Contoh yang baik: 'Kamu adalah asisten customer service toko Baju Kita yang ramah dan profesional. Jawab dalam Bahasa Indonesia, singkat, dan tidak menggunakan tanda bintang atau markdown. Jika tidak tahu jawaban, arahkan pelanggan menghubungi admin.'"},
                {"type": "step", "stepNumber": 5, "stepTitle": "Informasi Bisnis", "content": "Isi detail bisnis Anda: nama toko, alamat, jam operasional, nomor kontak, produk/layanan utama, metode pembayaran, dan kebijakan pengiriman. AI akan menggunakan informasi ini untuk menjawab pertanyaan pelanggan secara akurat."},
                {"type": "step", "stepNumber": 6, "stepTitle": "Temperature", "content": "Mengontrol kreativitas jawaban AI. Nilai 0.0-0.3: jawaban konsisten dan formal (cocok untuk CS). Nilai 0.5-0.7: seimbang antara konsisten dan natural. Nilai 0.8-1.0: lebih kreatif dan bervariasi. Rekomendasi default: 0.7."},
                {"type": "step", "stepNumber": 7, "stepTitle": "Max Tokens", "content": "Batas panjang maksimal balasan AI dalam token (≈ 0.75 kata per token). Nilai kecil (256-512): balasan singkat dan padat. Nilai besar (1024-2048): balasan lebih detail dan lengkap. Sesuaikan dengan gaya komunikasi bisnis Anda."},
                {"type": "step", "stepNumber": 8, "stepTitle": "AI Terpusat (Superadmin)", "content": "Superadmin dapat mengaktifkan mode AI Terpusat di menu AI Terpusat, di mana satu konfigurasi AI digunakan untuk semua user. Jika mode ini aktif, pengaturan AI di halaman ini akan diganti oleh konfigurasi terpusat."},
            ],
            "updatedAt": now_iso,
        },
        {
            ...
            "title": "Rules Engine",
            "sections": [
                {"type": "text", "content": "Rules Engine adalah sistem aturan balasan otomatis yang bekerja SEBELUM AI. Setiap pesan masuk dicek dulu ke rules — jika ada yang cocok, chatbot langsung membalas tanpa memanggil AI (lebih cepat dan hemat biaya). Rules sangat efektif untuk FAQ, menu, dan pertanyaan berulang."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Tambah Rule Baru", "content": "Klik tombol Tambah Rule (+ hijau). Isi: Nama Rule (identifikasi internal), Trigger/Kata Kunci (pemicu), Mode Respons, Teks Balasan, dan opsional Prioritas & URL Gambar."},
                {"type": "step", "stepNumber": 2, "stepTitle": "Jenis Trigger (Match Type)", "content": "Contains: pesan MENGANDUNG kata kunci di mana saja (paling fleksibel). Exact: pesan SAMA PERSIS dengan trigger (sensitif). Starts With: pesan DIAWALI kata kunci (cocok untuk menu: '1', '2', 'harga'). Regex: pola ekspresi reguler untuk trigger kompleks."},
                {"type": "step", "stepNumber": 3, "stepTitle": "Mode Respons", "content": "Direct: chatbot membalas langsung dengan teks yang Anda tulis — cepat, tidak butuh AI. AI Enhanced: AI menggunakan teks balasan Anda sebagai konteks tambahan untuk menghasilkan jawaban yang lebih natural dan lengkap."},
                {"type": "step", "stepNumber": 4, "stepTitle": "Prioritas Rule", "content": "Angka lebih kecil = diperiksa lebih dulu. Prioritas 1 diperiksa sebelum prioritas 10. Jika beberapa rule cocok, hanya rule dengan prioritas tertinggi (angka terkecil) yang digunakan. Atur prioritas untuk mencegah konflik antar rule."},
                {"type": "step", "stepNumber": 5, "stepTitle": "Kirim Gambar", "content": "Masukkan URL gambar publik (JPG/PNG) untuk dikirim bersama balasan teks. Cocok untuk mengirim katalog produk, peta lokasi, atau brosur otomatis saat trigger terpenuhi."},
                {"type": "step", "stepNumber": 6, "stepTitle": "Aktifkan / Nonaktifkan Rule", "content": "Toggle switch di setiap rule untuk mengaktifkan atau menonaktifkan tanpa menghapus. Berguna untuk promo terbatas waktu: nonaktifkan setelah promo selesai."},
                {"type": "step", "stepNumber": 7, "stepTitle": "Export & Import Rules", "content": "Tombol Export menyimpan semua rule ke file JSON. Tombol Import memuat rule dari file JSON — berguna untuk backup, migrasi antar akun, atau menduplikasi konfigurasi."},
                {"type": "step", "stepNumber": 8, "stepTitle": "Contoh Penggunaan", "content": "Rule 'Info Harga': trigger 'harga' (Contains), direct reply dengan daftar harga produk. Rule 'Jam Buka': trigger 'jam buka' atau 'buka jam' (Contains), direct reply dengan jadwal operasional. Rule 'Menu Utama': trigger '0' atau 'menu' (Contains), direct reply dengan daftar pilihan menu."},
            ],
            "updatedAt": now_iso,
        },
        {
            ...
            "sections": [
                {"type": "text", "content": "Knowledge Base adalah basis pengetahuan bisnis Anda yang digunakan AI untuk menjawab pertanyaan pelanggan secara akurat. Berbeda dengan Rules, Knowledge Base diproses oleh AI — cocok untuk informasi yang membutuhkan penjelasan panjang atau konteks variabel."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Tambah Entri Pengetahuan", "content": "Klik Tambah, lalu isi: Kategori (pengelompokan topik), Keyword (kata kunci pencarian), dan Konten (informasi detail). Klik Simpan. Entri langsung tersedia untuk AI."},
                {"type": "step", "stepNumber": 2, "stepTitle": "Kategori", "content": "Kelompokkan entri berdasarkan topik bisnis Anda. Contoh kategori: Produk (info produk dan varian), Harga (daftar harga dan promo), Pengiriman (biaya, estimasi, area), Pembayaran (metode dan rekening), Garansi (kebijakan garansi dan klaim), FAQ (pertanyaan umum lainnya)."},
                {"type": "step", "stepNumber": 3, "stepTitle": "Keyword Pencarian", "content": "Kata kunci yang memicu AI menggunakan entri ini. Bisa lebih dari satu kata kunci, pisahkan dengan koma. Contoh: 'harga, tarif, berapa harga, cost'. Semakin relevan keyword, semakin tepat AI menggunakan entri yang benar."},
                {"type": "step", "stepNumber": 4, "stepTitle": "Konten Pengetahuan", "content": "Tulis informasi selengkap mungkin. Contoh untuk entri 'Biaya Pengiriman': 'Pengiriman ke seluruh Indonesia menggunakan JNE, J&T, dan SiCepat. Biaya mulai dari Rp15.000 (Jabodetabek) hingga Rp35.000 (luar Jawa). Estimasi 1-3 hari kerja. Gratis ongkir untuk pembelian di atas Rp500.000.'"},
                {"type": "step", "stepNumber": 5, "stepTitle": "Aktif / Nonaktif", "content": "Toggle setiap entri untuk mengaktifkan atau menonaktifkan. Entri nonaktif tidak akan digunakan AI meski keyword-nya cocok. Berguna untuk menyembunyikan info musiman (promo Lebaran, dsb.) tanpa menghapus."},
                {"type": "step", "stepNumber": 6, "stepTitle": "Export & Import", "content": "Export semua entri ke JSON untuk backup. Import dari JSON untuk memuat entri secara massal — sangat efisien saat pertama kali mengisi Knowledge Base dengan banyak data."},
                {"type": "step", "stepNumber": 7, "stepTitle": "Alur Pemrosesan", "content": "Saat pesan masuk: (1) Rules Engine dicek dulu. (2) Jika tidak ada rule yang cocok, AI mencari entri Knowledge Base yang relevan berdasarkan keyword. (3) AI menggunakan konten yang ditemukan sebagai konteks tambahan untuk menjawab. (4) Jika tidak ada KB yang relevan, AI menjawab berdasarkan System Prompt dan Info Bisnis saja."},
            ],
            "updatedAt": now_iso,
        },
        {
            ...
            "title": "Template",
            "sections": [
                {"type": "text", "content": "Template adalah pesan siap pakai yang bisa dipilih langsung saat mengirim broadcast, menghemat waktu pengetikan dan memastikan konsistensi pesan. Template juga bisa dijadikan referensi balasan di Rules Engine."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Tambah Template Baru", "content": "Klik tombol Tambah Template. Isi Nama Template (label internal yang mudah dikenali), pilih Kategori, dan tulis Isi Pesan. Klik Simpan."},
                {"type": "step", "stepNumber": 2, "stepTitle": "Kategori Template", "content": "Kelompokkan template agar mudah ditemukan saat broadcast. Rekomendasi kategori: Sambutan (pesan selamat datang), Promo (penawaran dan diskon), Follow-up (tindak lanjut setelah transaksi), Pengingat (reminder pembayaran atau event), Informasi (update produk atau layanan)."},
                {"type": "step", "stepNumber": 3, "stepTitle": "Gunakan di Broadcast", "content": "Saat membuat broadcast, klik ikon Template untuk memilih pesan dari daftar template. Isi pesan otomatis terisi, Anda bisa edit sebelum kirim."},
                {"type": "step", "stepNumber": 4, "stepTitle": "Salin Isi Template", "content": "Klik ikon Salin pada template untuk menyalin teks ke clipboard. Paste langsung ke kolom balasan Rules Engine atau WhatsApp Anda."},
                {"type": "step", "stepNumber": 5, "stepTitle": "Edit & Hapus Template", "content": "Klik ikon Edit (pensil) untuk mengubah nama, kategori, atau isi template. Klik ikon Hapus (tempat sampah) untuk menghapus template yang tidak digunakan lagi."},
                {"type": "step", "stepNumber": 6, "stepTitle": "Tips Menulis Template", "content": "Gunakan sapaan personal jika mungkin. Tulis singkat dan jelas — pesan panjang cenderung tidak dibaca. Sertakan call-to-action yang jelas (contoh: 'Balas pesan ini untuk info lebih lanjut'). Hindari terlalu banyak emoji agar terlihat profesional."},
            ],
            "updatedAt": now_iso,
        },
        {
            ...
            "title": "Kontak",
            "sections": [
                {"type": "text", "content": "Halaman Kontak menyimpan semua nomor WhatsApp yang pernah berinteraksi dengan chatbot. Data kontak digunakan untuk broadcast terarah, analisis pelanggan, dan manajemen daftar hitam."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Kontak Tersimpan Otomatis", "content": "Setiap nomor yang mengirim pesan pertama kali ke chatbot otomatis tersimpan sebagai kontak baru. Data yang disimpan: nomor WhatsApp (Chat ID), nama (dari profil WA jika tersedia), waktu pertama dan terakhir berinteraksi, jumlah pesan."},
                {"type": "step", "stepNumber": 2, "stepTitle": "Edit Nama & Catatan", "content": "Klik ikon Edit pada kontak untuk mengubah nama tampilan dan menambahkan catatan internal (tidak terlihat pelanggan). Contoh catatan: 'Pelanggan VIP', 'Sering komplain pengiriman', 'Reseller Bandung'."},
                {"type": "step", "stepNumber": 3, "stepTitle": "Sistem Tag", "content": "Tambahkan tag ke kontak untuk pengelompokan. Contoh tag: VIP, Reseller, Pelanggan Baru, Zona Jakarta, Belum Bayar. Tag bisa digunakan sebagai filter target saat broadcast."},
                {"type": "step", "stepNumber": 4, "stepTitle": "Blokir Kontak", "content": "Aktifkan toggle Blokir pada kontak tertentu agar chatbot tidak membalas pesan dari nomor tersebut. Kontak yang diblokir tetap tersimpan di daftar namun semua pesannya akan diabaikan."},
                {"type": "step", "stepNumber": 5, "stepTitle": "Cari & Filter Kontak", "content": "Gunakan kolom pencarian untuk mencari berdasarkan nama atau nomor. Filter dropdown untuk menyaring berdasarkan tag atau status (aktif/diblokir)."},
                {"type": "step", "stepNumber": 6, "stepTitle": "Hapus Kontak", "content": "Klik ikon Hapus untuk menghapus kontak dari daftar. Riwayat pesan dari kontak tersebut tetap tersimpan di halaman Pesan."},
                {"type": "step", "stepNumber": 7, "stepTitle": "Export Kontak", "content": "Tombol Export memungkinkan mengunduh seluruh daftar kontak dalam format CSV atau Excel. Berguna untuk analisis data pelanggan di luar sistem atau import ke CRM lain."},
            ],
            "updatedAt": now_iso,
        },
        {
            ...
            "title": "Pesan",
            "sections": [
                {"type": "text", "content": "Halaman Pesan menampilkan riwayat lengkap semua pesan yang masuk dari pelanggan dan balasan yang dikirim chatbot. Gunakan halaman ini untuk memantau percakapan, mengecek kualitas balasan, dan menemukan pesan yang perlu tindak lanjut."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Tampilan Riwayat Pesan", "content": "Pesan ditampilkan secara kronologis dari terbaru. Setiap entri menampilkan: arah pesan (masuk/keluar), nomor pengirim, preview isi pesan, timestamp, dan jenis balasan (Direct Rule / AI / System)."},
                {"type": "step", "stepNumber": 2, "stepTitle": "Ikon Jenis Pesan", "content": "Panah masuk (biru): pesan dari pelanggan. Panah keluar (hijau): balasan chatbot via Rules. Panah keluar (ungu): balasan chatbot via AI. Tanda seru (merah): pesan gagal dikirim atau error."},
                {"type": "step", "stepNumber": 3, "stepTitle": "Cari Percakapan", "content": "Gunakan kolom pencarian untuk mencari berdasarkan nomor WhatsApp atau kata kunci dalam isi pesan. Filter tambahan tersedia untuk menyaring hanya pesan masuk, hanya balasan, atau berdasarkan rentang waktu."},
                {"type": "step", "stepNumber": 4, "stepTitle": "Detail Pesan Lengkap", "content": "Klik pada baris pesan untuk melihat isi lengkap. Untuk pesan panjang yang dipotong di tampilan daftar, detail menampilkan pesan utuh beserta metadata (rule yang digunakan, model AI, token yang dipakai jika via AI)."},
                {"type": "step", "stepNumber": 5, "stepTitle": "Pengaturan Retensi Pesan", "content": "Secara default, pesan disimpan selama 90 hari. Setelah itu dihapus otomatis untuk menjaga performa database. Ubah durasi retensi di menu Setting → Retensi Data sesuai kebutuhan."},
                {"type": "step", "stepNumber": 6, "stepTitle": "Export Pesan", "content": "Gunakan tombol Export untuk mengunduh riwayat pesan dalam format CSV. Berguna untuk audit, laporan bulanan, atau analisis pola percakapan pelanggan."},
            ],
            "updatedAt": now_iso,
        },
        {
            ...
            "title": "Broadcast",
            "sections": [
                {"type": "text", "content": "Fitur Broadcast memungkinkan pengiriman pesan massal ke banyak kontak sekaligus melalui WhatsApp. Gunakan untuk promosi, pengumuman, follow-up, atau reminder. Pesan dikirim satu per satu (bukan grup) sehingga terasa personal."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Pilih Target Penerima", "content": "Tiga opsi target: Semua Kontak (semua nomor di daftar kontak kecuali yang diblokir), By Tag (hanya kontak dengan tag tertentu — contoh: kirim promo hanya ke tag 'Reseller'), Custom (masukkan nomor manual, satu per baris, format: 628xxx)."},
                {"type": "step", "stepNumber": 2, "stepTitle": "Tulis atau Pilih Pesan", "content": "Tulis pesan di kolom teks atau klik ikon Template untuk memilih dari daftar template yang sudah dibuat. Pesan mendukung format WhatsApp: *tebal*, _miring_, ~coret~, dan emoji."},
                {"type": "step", "stepNumber": 3, "stepTitle": "Cek Sebelum Kirim (Wajib)", "content": "Klik tombol Cek untuk melihat: jumlah total penerima, estimasi waktu pengiriman, dan preview pesan. Pastikan semua sudah benar sebelum melanjutkan ke langkah kirim."},
                {"type": "step", "stepNumber": 4, "stepTitle": "Proses Pengiriman", "content": "Klik Kirim setelah mengkonfirmasi. Pesan dikirim secara bertahap (batch) dengan jeda antar pesan untuk menghindari deteksi spam WhatsApp. Progress bar menampilkan status pengiriman real-time."},
                {"type": "step", "stepNumber": 5, "stepTitle": "Batas & Pengaturan", "content": "Default: maksimal 100 pesan per hari, 10 pesan per batch, jeda 3-5 detik antar pesan. Batas ini bisa diubah di Setting → Broadcast. Hindari mengirim broadcast lebih dari 200 pesan/hari untuk menjaga keamanan akun WhatsApp."},
                {"type": "step", "stepNumber": 6, "stepTitle": "Kirim Pesan Individual", "content": "Gunakan tombol Kirim Satu untuk mengirim pesan ke satu nomor spesifik tanpa melalui flow broadcast. Berguna untuk follow-up cepat ke kontak tertentu."},
                {"type": "step", "stepNumber": 7, "stepTitle": "Praktik Terbaik Broadcast", "content": "Kirim hanya ke kontak yang pernah berinteraksi. Sertakan opsi untuk tidak menerima pesan lagi (opt-out). Hindari jam malam (22.00-07.00). Personalisasi pesan sebisa mungkin. Jangan kirim broadcast promosi lebih dari 3x seminggu ke kontak yang sama."},
            ],
            "updatedAt": now_iso,
        },
        {
            ...
            "title": "AI Setup",
            "sections": [
                {"type": "text", "content": "AI Setup adalah fitur konfigurasi cerdas yang membantu Anda menyiapkan chatbot hanya dengan bercerita tentang bisnis Anda. AI akan otomatis menghasilkan System Prompt, Knowledge Base, dan Rules yang sesuai — tanpa perlu keahlian teknis."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Cara Memulai", "content": "Buka menu AI Setup dan mulai chat dengan AI. Perkenalkan bisnis Anda dengan natural: 'Halo, saya punya toko baju online bernama Baju Kita, menjual pakaian wanita. Pelanggan sering tanya soal ukuran, pengiriman, dan metode bayar.'"},
                {"type": "step", "stepNumber": 2, "stepTitle": "AI Generate Konfigurasi", "content": "AI akan mengajukan pertanyaan klarifikasi jika perlu, lalu menghasilkan: System Prompt (kepribadian chatbot), entri Knowledge Base (FAQ bisnis Anda), dan Rules (aturan balasan otomatis untuk pertanyaan umum)."},
                {"type": "step", "stepNumber": 3, "stepTitle": "Review Hasil", "content": "Hasil konfigurasi ditampilkan dalam panel di sebelah kanan. Baca dan periksa setiap bagian. Jika ada yang perlu diubah, minta AI untuk merevisi melalui chat ('Ubah system prompt agar lebih formal', 'Tambahkan info metode pembayaran COD')."},
                {"type": "step", "stepNumber": 4, "stepTitle": "Apply Konfigurasi", "content": "Klik tombol Apply untuk menerapkan hasil ke chatbot. System prompt akan diperbarui di AI Agent, entri Knowledge Base ditambahkan, dan Rules baru dibuat. Proses ini tidak menghapus konfigurasi yang sudah ada, melainkan menambahkan."},
                {"type": "step", "stepNumber": 5, "stepTitle": "Iterasi & Penyempurnaan", "content": "Lanjutkan percakapan untuk menyempurnakan. Contoh: 'Tambahkan rule untuk sapaan selamat pagi', 'Buat chatbot lebih santai dan gunakan kata ganti lo-gue', 'Tambahkan info promo gratis ongkir di atas 500rb'."},
                {"type": "step", "stepNumber": 6, "stepTitle": "Catatan Penting", "content": "AI Setup membutuhkan konfigurasi AI Agent yang sudah aktif (provider dan API Key sudah diisi). Hasil AI Setup adalah titik awal — Anda tetap disarankan untuk mengecek dan menyempurnakan hasil di menu AI Agent, Rules Engine, dan Knowledge Base."},
            ],
            "updatedAt": now_iso,
        },
        {
            ...
            "title": "Test Center",
            "sections": [
                {"type": "text", "content": "Test Center adalah alat pengujian untuk memastikan chatbot bekerja seperti yang diharapkan sebelum digunakan pelanggan. Uji rules, knowledge base, dan alur lengkap tanpa mengirim pesan WhatsApp sungguhan."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Test Rule", "content": "Masukkan pesan di tab Test Rule dan klik Uji. Sistem mencari apakah ada rule yang cocok dengan pesan tersebut. Hasil menampilkan: rule yang terpicu (nama dan trigger), teks balasan, dan mode respons (Direct/AI Enhanced). Jika tidak ada yang cocok, ditampilkan 'Tidak ada rule yang cocok'."},
                {"type": "step", "stepNumber": 2, "stepTitle": "Test Knowledge Base", "content": "Di tab Test Knowledge, masukkan pertanyaan pelanggan dan klik Uji. Sistem menampilkan entri Knowledge Base yang relevan beserta skor relevansi. Gunakan ini untuk memastikan keyword KB Anda sudah tepat menangkap pertanyaan yang sering diajukan."},
                {"type": "step", "stepNumber": 3, "stepTitle": "Test Full Flow", "content": "Tab Test Full Flow menyimulasikan alur lengkap pemrosesan pesan: (1) Cek Rules → (2) Cek Knowledge Base → (3) Panggil AI → (4) Hasilkan balasan. Ini adalah test paling komprehensif dan paling mendekati kondisi nyata."},
                {"type": "step", "stepNumber": 4, "stepTitle": "Test AI Langsung", "content": "Tab Test AI memungkinkan mengirim pesan langsung ke AI dengan system prompt dan info bisnis yang sudah dikonfigurasi, tanpa melalui Rules dan KB. Berguna untuk menguji kualitas dan kepribadian AI secara terisolasi."},
                {"type": "step", "stepNumber": 5, "stepTitle": "Membaca Hasil Test", "content": "Hasil Full Flow menampilkan detail setiap tahap: rule yang terpicu (jika ada), KB yang digunakan (kategori dan konten), prompt yang dikirim ke AI, dan balasan final. Warna hijau = berhasil, merah = error, abu = tidak digunakan."},
                {"type": "step", "stepNumber": 6, "stepTitle": "Kapan Perlu Test", "content": "Selalu test setelah: menambah atau mengubah rule baru, menambah entri Knowledge Base, mengubah system prompt, atau mengubah model/provider AI. Test juga saat menerima laporan chatbot memberikan jawaban yang salah atau tidak relevan."},
            ],
            "updatedAt": now_iso,
        },
        {
            ...
            "title": "Logs",
            "sections": [
                {"type": "text", "content": "Halaman Logs mencatat semua aktivitas sistem chatbot secara detail. Gunakan untuk monitoring operasional, debugging masalah, audit keamanan, dan memahami pola penggunaan."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Jenis Event Log", "content": "LOG_IN / LOGIN_FAILED: aktivitas autentikasi. MESSAGE_IN: pesan masuk dari pelanggan. MESSAGE_OUT: balasan yang dikirim chatbot. RULE_HIT: rule yang terpicu beserta nama rule. AI_CALL: panggilan ke AI (provider, model, token). KB_MATCH: Knowledge Base yang digunakan. LICENSE_ACTIVATED: aktivasi lisensi. USER_CREATED: pembuatan user baru. CONFIG_UPDATED: perubahan konfigurasi."},
                {"type": "step", "stepNumber": 2, "stepTitle": "Filter & Pencarian", "content": "Gunakan dropdown jenis event untuk menyaring log berdasarkan tipe aktivitas. Kolom pencarian teks untuk mencari berdasarkan isi pesan, username, atau keterangan log. Filter rentang waktu untuk melihat log pada periode tertentu."},
                {"type": "step", "stepNumber": 3, "stepTitle": "Detail Log", "content": "Klik baris log untuk melihat detail lengkap: timestamp presisi, user yang terlibat, nomor kontak, isi pesan (untuk log pesan), nama rule atau KB yang digunakan, dan kode error jika ada."},
                {"type": "step", "stepNumber": 4, "stepTitle": "Retensi & Pembersihan Log", "content": "Log tersimpan selama 30 hari secara default. Ubah durasi di Setting → Retensi Data. Tombol Reset Log (di halaman Reset Data) menghapus semua log secara manual. Log lama terhapus otomatis saat batas retensi tercapai."},
                {"type": "step", "stepNumber": 5, "stepTitle": "Menggunakan Log untuk Debug", "content": "Jika chatbot tidak membalas: cari log MESSAGE_IN untuk memastikan pesan diterima, cek apakah ada RULE_HIT, lalu cek AI_CALL — jika tidak ada, berarti ada masalah di konfigurasi koneksi atau AI. Jika ada log error, salin pesan error untuk diserahkan ke support."},
            ],
            "updatedAt": now_iso,
        },
        {
            ...
            "title": "Setting",
            "sections": [
                {"type": "text", "content": "Halaman Setting berisi pengaturan operasional chatbot yang mempengaruhi perilaku bot secara keseluruhan. Perubahan tersimpan otomatis dan langsung berlaku tanpa perlu restart."},
                {"type": "step", "stepNumber": 1, "stepTitle": "Bot Aktif / Nonaktif", "content": "Toggle utama di bagian atas untuk mengaktifkan atau menonaktifkan chatbot. Saat nonaktif, semua pesan masuk diabaikan (tidak dibalas). Indikator 'Bot Aktif' di header akan berubah menjadi merah saat nonaktif."},
                {"type": "step", "stepNumber": 2, "stepTitle": "Jam Kerja", "content": "Aktifkan jam kerja agar bot hanya merespons pada jam yang ditentukan. Atur jam mulai dan jam selesai. Di luar jam kerja, bot akan mengirim Pesan Offline yang bisa Anda kustomisasi. Contoh: 'Maaf, kami tutup. Kami akan membalas pada hari kerja pukul 08.00-17.00 WIB.'"},
                {"type": "step", "stepNumber": 3, "stepTitle": "Simulasi Mengetik", "content": "Aktifkan agar bot mengirim status 'sedang mengetik...' selama beberapa detik sebelum membalas. Membuat interaksi terasa lebih natural seperti dibalas manusia. Durasi typing disesuaikan otomatis dengan panjang balasan."},
                {"type": "step", "stepNumber": 4, "stepTitle": "Delay Respons", "content": "Atur jeda waktu (dalam milidetik) antara pesan masuk dan balasan dikirim. Default: 2000ms (2 detik). Nilai lebih tinggi (3000-5000ms) membuat bot terasa lebih natural. Nilai terlalu rendah (< 500ms) bisa terasa seperti bot."},
                {"type": "step", "stepNumber": 5, "stepTitle": "Rate Limit per Kontak", "content": "Batasi jumlah maksimal balasan ke satu nomor dalam satu menit. Default: 15 pesan/menit. Mencegah loop balasan tak terbatas jika ada bug atau kontak yang mengirim pesan berulang sangat cepat."},
                {"type": "step", "stepNumber": 6, "stepTitle": "Retensi Data", "content": "Atur berapa lama data disimpan sebelum dihapus otomatis: Retensi Pesan (default 90 hari), Retensi Log (default 30 hari). Nilai lebih kecil = database lebih ringan. Nilai lebih besar = riwayat lebih panjang."},
                {"type": "step", "stepNumber": 7, "stepTitle": "Pengaturan Broadcast", "content": "Batas harian broadcast: maksimal pesan yang bisa dikirim per hari (default 100). Ukuran batch: jumlah pesan per gelombang pengiriman (default 10). Jeda antar batch: waktu tunggu antar gelombang (default 5 detik). Naikkan batas ini hanya jika akun WhatsApp Anda sudah terverifikasi bisnis."},
                {"type": "step", "stepNumber": 8, "stepTitle": "Ubah Password", "content": "Tombol Ubah Password tersedia di bagian bawah halaman Setting. Masukkan password lama, password baru (minimal 8 karakter, harus mengandung huruf dan angka), dan konfirmasi password baru."},
            ],
            "updatedAt": now_iso,
        },
    ]
